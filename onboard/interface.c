...
// Replace this (and the serialSendChar and serialSend32Bit functions) with matching implementations from your own project.
#include "serial.h"

#define U32_FROM_MSG(msg, index)                                                                           \
    (uint32_t)((uint32_t)msg[index] + ((uint32_t)msg[index + 1] << 8) + ((uint32_t)msg[index + 2] << 16) + \
...
void vscopeProcessMessage(char* msg) {
    ...
    switch (msg[0]) {
        ...
        case GET_BUFF: {
            ...
            float value = (address < RT_BUFFER_LENGTH) ? vscopeGetRtBuffer((VscopeRtBufferIndexes)address) : 0.0f;
        ...
        case GET_LABEL: {
            ...
            if (label < VSCOPE_NUM_CHANNELS) {
                for (i = 0U; (i < VSCOPE_MAX_NAME_LEN) && (vscope_channel_names[label][i] != '\0'); i += 1U) {
